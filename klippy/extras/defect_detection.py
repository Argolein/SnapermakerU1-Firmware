...
class DefectDetection:
    def __init__(self, config):
        ...
        self.bed_detect_probe_times = config.getint('bed_detect_probe_times', 3)
        self.bed_detect_probe_distance = config.getfloat('bed_detect_probe_distance', 90.0)
    ...
    def detect_dirty_bed(self, json_obj):
        ...
        if len(self.check_dirty_bed_result) >= self.config['clean_bed']['check_window']:
            self.check_dirty_bed_result.pop(0)

        if json_obj is None:
            self.check_dirty_bed_result.append(False)
            return False
        ...
        if probability >= detect_threshold:
            self.check_dirty_bed_result.append(True)
            logging.info("[defect_detection] detected dirty bed, times: %d",
                            self.check_dirty_bed_result.count(True))
        else:
            self.check_dirty_bed_result.append(False)
            return False

        if self.check_dirty_bed_result.count(True) >= self.config['clean_bed']['check_window'] * detect_factor:
            self.check_dirty_bed_result = []
            return True
        else:
            return False
    ...
    def request_detect_clean_bed_sync(self, ignore_ratio=None):
        try:
            if self.mqtt_client is None:
                return False
    ...
    def cmd_DEFECT_DETECTION_DETECT_BED(self, gcmd):
        ...
        safety_z = 0
        is_home_z =  bool('z' in homed_axes_list)
        ...
        try:
            ...
            if is_home_z:
                for i in range(self.config['clean_bed']['check_window']):
                    logging.info("[defect_detection] detect bed times: %d", i + 1)
                    self.gcode.run_script_from_command(f"G1 Z{7+i*3}")
                    toolhead.wait_moves()
                    if self.request_detect_clean_bed_sync():
                ...
                for probe_times in range(self.bed_detect_probe_times):
                    try:
                    ...
                    for i in range(self.config['clean_bed']['check_window']):
                        ...
                        if safety_z > 0:
                            if i % 2 != 0:
                                dest_z = min(start_pos[2], min(current_pos[2] + safety_z, current_pos[2] + 5))
                                toolhead.manual_move([None, None, dest_z], 30)
                            else:
                                dest_z = min(start_pos[2], min(current_pos[2] + safety_z, current_pos[2] + 3))
                                toolhead.manual_move([None, None, dest_z], 30)

                        ignore_ratio = max(1.0 - (probe_times + 1.0) / self.bed_detect_probe_times, 0)
                        if self.request_detect_clean_bed_sync(ignore_ratio):
                            toolhead.get_kinematics().note_z_not_homed()
                            self.ignore_detect_bed = True
                            self.defect_event_handler(CONFIRM_DIRTY_BED, from_command=True)
